...
SolverResults* scip_solver(
    ...
    if (run) {
        ...
        if (init_w != NULL && initial_xs != NULL && initial_ys != NULL) {
            SCIP_SOL *sol;
            retcode = SCIPcreateSol(scip, &sol, NULL);
            
            // Set x variables
            for (int i = 0; i < num_positive; i++) {
                retcode = SCIPsetSolVal(scip, sol, x_vars[i], (double)initial_xs[i]);
            }
            
            // Set y variables
            for (int j = 0; j < num_negative; j++) {
                retcode = SCIPsetSolVal(scip, sol, y_vars[j], (double)initial_ys[j]);
            }
            
            // Set w variables
            for (int d = 0; d < n_features; d++) {
                retcode = SCIPsetSolVal(scip, sol, w_vars[d], init_w[d]);
            }
            
            // Set c variable
            retcode = SCIPsetSolVal(scip, sol, c_var, init_c);
            ...
            retcode = SCIPsetSolVal(scip, sol, V_var, v_val);
            
            printf("Applying initial solution with reach=%.2f\n", initial_reach);
            
            // Try to add the solution
            SCIP_Bool stored;
            retcode = SCIPaddSolFree(scip, &sol, &stored);
            
            if (!stored) {
                printf("Warning: Could not add initial solution as heuristic\n");
            }
        }
...
CLEANUP:
    ...
    if (x_vars) {
        for (int i = 0; i < num_positive; i++) {
            if (x_vars[i]) SCIPreleaseVar(scip, &x_vars[i]);
        }
        free(x_vars);
    }
    ...
    if (y_vars) {
        for (int j = 0; j < num_negative; j++) {
            if (y_vars[j]) SCIPreleaseVar(scip, &y_vars[j]);
        }
        free(y_vars);
    }
    ...
    if (w_vars) {
        for (int d = 0; d < n_features; d++) {
            if (w_vars[d]) SCIPreleaseVar(scip, &w_vars[d]);
        }
        free(w_vars);
    }
    
    if (c_var) SCIPreleaseVar(scip, &c_var);
    if (V_var) SCIPreleaseVar(scip, &V_var);
    ...
    // Free temporary data
    free_matrix(X);
    if (initial_xs) free(initial_xs);
    if (initial_ys) free(initial_ys);
...
void free_solver_results(SolverResults *results) {
    if (results) {
        if (results->hyperplane_w) free(results->hyperplane_w);
        if (results->x_vals) free(results->x_vals);
        if (results->y_vals) free(results->y_vals);
        free(results);
    }
}
