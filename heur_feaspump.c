...
static
SCIP_RETCODE logVariablesToCSV(
   SCIP*                 scip,               /**< SCIP data structure */
   const char*           filename,           /**< CSV filename */
   const char*           stage,              /**< stage identifier (e.g., "LP", "ROUNDED") */
   int                   iteration,          /**< current iteration number */
   SCIP_VAR**            vars,               /**< array of variables */
   int                   nvars,              /**< number of variables */
   SCIP_SOL*             sol                 /**< solution to log (NULL for LP solution) */
   )
{
   FILE* file;
   int i;
   ...
   if( file == NULL )
   {
      SCIPwarningMessage(scip, "Could not open file %s for writing\n", filename);
      return SCIP_OKAY;
   }
   ...
   if( !fileexists )
   {
      fprintf(file, "Stage,Iteration");
      for( i = 0; i < nvars; i++ )
      {
         fprintf(file, ",%s", SCIPvarGetName(vars[i]));
      }
      fprintf(file, "\n");
   }
   
   /* write variable values */
   fprintf(file, "%s,%d", stage, iteration);
   for( i = 0; i < nvars; i++ )
   {
      SCIP_Real value;
      if( sol != NULL )
...
/** logs detailed variable information including fractionality */
static
SCIP_RETCODE logDetailedVariablesToCSV(
   SCIP*                 scip,               /**< SCIP data structure */
   const char*           filename,           /**< CSV filename */
   const char*           stage,              /**< stage identifier */
   int                   iteration,          /**< current iteration number */
   SCIP_VAR**            vars,               /**< array of variables */
   int                   nvars,              /**< number of variables */
   SCIP_SOL*             sol                 /**< solution to log (NULL for LP solution) */
   )
{
   ...
   if( !fileexists )
   {
      ...
      for( i = 0; i < nvars; i++ )
      {
         fprintf(file, ",%s_value,%s_frac", SCIPvarGetName(vars[i]), SCIPvarGetName(vars[i]));
      }
      fprintf(file, "\n");
   }
   
   /* write variable values and fractionality */
   fprintf(file, "%s,%d", stage, iteration);
   for( i = 0; i < nvars; i++ )
   {
      SCIP_Real value;
      ...
      {
         value = SCIPgetSolVal(scip, sol, vars[i]);
         /* for rounded solutions, calculate fractionality from original LP values */
         if( strcmp(stage, "ROUNDED") == 0 )
            frac = 0.0; /* rounded values should be integral */
         else
            frac = SCIPfeasFrac(scip, value);
      }
      else
      {
         value = SCIPvarGetLPSol(vars[i]);
...
SCIP_DECL_HEUREXEC(heurExecFeaspump)
{
   ...
   *result = SCIP_DIDNOTFIND;
      ...
      && !SCIPisStopped(scip) )
   {
      ...
      SCIP_CALL( logVariablesToCSV(scip, "feaspump_variables.csv", "ROUNDED", nloops, vars, nvars, heurdata->roundedsol) );
      ...
      {
         /* Choose one of these two logging options: */
         
         /* Option 1: Basic logging */
         SCIP_CALL( logVariablesToCSV(scip, "feaspump_variables.csv", "LP", nloops, vars, nvars, NULL) );
         
         /* Option 2: Detailed logging with fractionality (comment out Option 1 if using this) */
         /* SCIP_CALL( logDetailedVariablesToCSV(scip, "feaspump_detailed.csv", "LP", nloops, vars, nbinvars + nintvars, NULL) ); */
      }
