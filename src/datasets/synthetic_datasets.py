...
from src.constants import D_MAX
import numpy as np
...
class TruncatedNormalPrism(Dataset):
    ...
    def _generate(self):
        ...
        num_cluster_positive = int(
            self.num_positive * (1 - self.positive_background_ratio)
        )
        ...
        self.X_positive = (
            np.vstack((X_cluster, X_background)) if X_background.size > 0 else X_cluster
        )

        # Combine all samples and create labels
        self.X = np.vstack((self.X_negative, self.X_positive))
        self.y = np.concatenate(
            (np.zeros(self.num_negative), np.ones(self.num_positive))
        )
