...
class GurobiSolver:
    ...
    def solve(self):
        ...
        if self.mdl.status in [grb.GRB.OPTIMAL, grb.GRB.TIME_LIMIT]:
            ...
            x_values = [self.x_vars[i].x for i in range(len(self.P))]
            print(len(x_values))

            return int(np.sum(x_values)), int(node_count), self.mdl
        else:
    ...
    def solve(self):
        ...
        try:
            ...
            if status in [
                ...
                # Calculate `reach`
                reach = sum(
                    1 for s in self.P if np.dot(w_values, s) < self.epsilon_P + c_value
                )
                print(reach, np.sum(x_values))
