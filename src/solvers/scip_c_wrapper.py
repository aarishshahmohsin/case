...
class SCIPSolver:
    ...
    def _extract_results(self, results_ptr, n_features, n_positive, n_negative):
        ...
        if results.status < 0:
            error_msg = results.error_msg.decode('utf-8') if results.error_msg else "Unknown error"
        ...
        return {
            ...
            "Status": "Optimal" if results.status == 0 else "Time limit" if results.status == 1 else "Error"
        }
    
    def solve(self, P, N, init_w, init_c, theta, lambda_param=None, 
              epsilon_P=0.01, epsilon_N=0.01, epsilon_R=0.001, 
              theta0=0.1, theta1=0.2, dataset_name="test", 
              print_output=False, time_limit=300.0):
        """
        ...
        try:
            ...
            # Extract results
            results = self._extract_results(results_ptr, P.shape[1], P.shape[0], N.shape[0])
...
def call_scip_solver(P, N, init_w, init_c, theta, lambda_param=None,
                     epsilon_P=0.01, epsilon_N=0.01, epsilon_R=0.001,
                     **kwargs):
    """
    ...
    return _solver.solve(
        P=P, N=N, init_w=init_w, init_c=init_c, theta=theta,
        lambda_param=lambda_param, epsilon_P=epsilon_P, 
        epsilon_N=epsilon_N, epsilon_R=epsilon_R, **kwargs
    )
...
if __name__ == "__main__":
    ...
    try:
        results = call_scip_solver(
            P=P, N=N, init_w=init_w, init_c=init_c, 
            theta=0.8, lambda_param=None,
            epsilon_P=0.01, epsilon_N=0.01, epsilon_R=0.001
        )
