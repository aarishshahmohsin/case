...
struct SCIP_HeurData
{
   ...
   char                  logfilename[LOG_FILENAME_MAX]; /**< log file name */
   int                   iteration;          /**< current iteration counter */
   SCIP_Bool             logging_enabled;    /**< whether logging is enabled */
};
...
/** creates log filename with timestamp */
static
void createLogFilename(
   SCIP_HEURDATA*        heurdata,           /**< heuristic data */
   const char*           prefix              /**< filename prefix */
   )
{
   time_t rawtime;
   struct tm* timeinfo;
   ...
   snprintf(heurdata->logfilename, LOG_FILENAME_MAX, 
      "%s_intshifting_%04d%02d%02d_%02d%02d%02d.csv",
...
static
SCIP_RETCODE initializeLogging(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_HEURDATA*        heurdata,           /**< heuristic data */
   ...
   )
{
   ...
   /* Create log filename */
   createLogFilename(heurdata, log_prefix);
   
   /* Open log file */
   heurdata->logfile = fopen(heurdata->logfilename, "w");
   if( heurdata->logfile == NULL )
   {
      SCIPwarningMessage(scip, "Could not open log file %s\n", heurdata->logfilename);
      heurdata->logging_enabled = FALSE;
      return SCIP_OKAY;
   }
   
   heurdata->logging_enabled = TRUE;
   heurdata->iteration = 0;
   
   /* Write CSV header */
   fprintf(heurdata->logfile, "Stage,Iteration");
   
   /* Get all variables and write their names to header */
   vars = SCIPgetVars(scip);
   nvars = SCIPgetNVars(scip);
   
   for( v = 0; v < nvars; ++v )
   {
      fprintf(heurdata->logfile, ",%s", SCIPvarGetName(vars[v]));
   }
   fprintf(heurdata->logfile, "\n");
   fflush(heurdata->logfile);
   
   SCIPinfoMessage(scip, NULL, "Intshifting logging enabled. Log file: %s\n", heurdata->logfilename);
   
   return SCIP_OKAY;
}

/** logs current variable values */
static
SCIP_RETCODE logVariableValues(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_HEURDATA*        heurdata,           /**< heuristic data */
   SCIP_SOL*             sol,                /**< current solution */
   const char*           stage,              /**< current stage description */
   SCIP_VAR*             changed_var,        /**< variable that was just changed (can be NULL) */
   SCIP_Real             old_val,            /**< old value of changed variable */
   SCIP_Real             new_val             /**< new value of changed variable */
   )
{
   SCIP_VAR** vars;
   int nvars;
   ...
   if( !heurdata->logging_enabled || heurdata->logfile == NULL )
      return SCIP_OKAY;
   
   vars = SCIPgetVars(scip);
   nvars = SCIPgetNVars(scip);
   
   /* Write stage and iteration */
   fprintf(heurdata->logfile, "%s,%d", stage, heurdata->iteration);
   
   /* Write all variable values */
   for( v = 0; v < nvars; ++v )
   {
      SCIP_Real val = SCIPgetSolVal(scip, sol, vars[v]);
      fprintf(heurdata->logfile, ",%.6f", val);
   }
   
   fprintf(heurdata->logfile, "\n");
   fflush(heurdata->logfile);
   
   /* Log change information to console if variable changed */
   if( changed_var != NULL )
   {
      SCIPdebugMsg(scip, "INTSHIFT_LOG: Iter %d - Variable %s: %.6f -> %.6f (stage: %s)\n", 
         heurdata->iteration, SCIPvarGetName(changed_var), old_val, new_val, stage);
   }
   ...
   )
{
   if( heurdata->logging_enabled && heurdata->logfile != NULL )
   {
      fclose(heurdata->logfile);
      heurdata->logfile = NULL;
      heurdata->logging_enabled = FALSE;
   }
}

/* Modify heurInitIntshifting function */


/* Modify heurExitIntshifting function */
...
{  /*lint --e{715}*/
   ...
   /* Initialize logging - you can control this with a parameter or environment variable */
   if( getenv("SCIP_LOG_INTSHIFTING") != NULL || SCIPgetVerbLevel(scip) >= SCIP_VERBLEVEL_HIGH )
   {
      SCIP_CALL( initializeLogging(scip, heurdata, "intshift_vars") );
   }
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   if( heurdata->logging_enabled )
   {
      heurdata->iteration = 0;
      SCIP_CALL( logVariableValues(scip, heurdata, sol, "INITIAL", NULL, 0.0, 0.0) );
   }
   ...
   while( (nfrac > 0 || nviolrows > 0) && nnonimprovingshifts < MAXSHIFTINGS && !SCIPisStopped(scip) )
   {
      ...
      {
         heurdata->iteration++;
         SCIP_CALL( logVariableValues(scip, heurdata, sol, "SHIFTED", shiftvar, oldsolval, newsolval) );
      }
   ...
   if( nfrac == 0 && nviolrows == 0 )
   {
      ...
      /* Log state before LP solve */
      if( heurdata->logging_enabled )
      {
         SCIP_CALL( logVariableValues(scip, heurdata, sol, "PRE_LP", NULL, 0.0, 0.0) );
      }
      ...
      if( !lperror && SCIPgetLPSolstat(scip) == SCIP_LPSOLSTAT_OPTIMAL )
      {
         ...
         if( stored && heurdata->logging_enabled )
         {
            SCIP_CALL( logVariableValues(scip, heurdata, sol, "SOLUTION", NULL, 0.0, 0.0) );
         }
      }
